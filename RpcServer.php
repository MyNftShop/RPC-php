...
class RpcServer {
    ...
    public function __construct($host, $port, $path) {
        ...
        qDebug(sprintf("%s:%d start!",$host,$port));
            ...
            if ($client) {
                ...
                if($classRet && $methodRet) {
                    ...
                    if(file_exists($file)) {
                        ...
                        //如果有参数，则传入指定参数
                        if(!$paramsRet) {
                            qDebug(sprintf("call: %s->%s()",$class,$method[1]));
                            $data = $obj->$method[1]();
                        } else {
                            qDebug(sprintf("call: %s->%s(%s)",$class,$method[1],$params[1]));
